...
void LearnCharComponentsFromFile()
{
    ...
    while((read = getline(&line, &len, fin)) != -1)
    {
        ...
        int tmp_cnt = 0;
        pch = strtok_r(NULL, " \n", &save_ptr);
        int maxNum = MAX_CHINESE - MIN_CHINESE;
        int tmp = 0;
        while(pch != NULL)
        {
            mbstowcs(wstr, pch, MAX_STRING);
            tmp = wstr[0] - MIN_COMP;
            if(wstr[0] > MAX_COMP || wstr[0] < MIN_COMP)
                tmp = 3;
            char2comp[id].comp[tmp_cnt++] = wstr[0] - MIN_CHINESE;
            pch = strtok_r(NULL, " \n", &save_ptr);
        }
    }
    if(line)
...
void InitNet()
{
    ...
    a = posix_memalign((void **) &synchar, 128, (long long) COMP_SIZE * layer1_size * sizeof(real));
    if(synchar == NULL)
    {
        printf("Memory allocation failed\n");
        exit(1);
        ...
        for(a = b; a < window * 2 + 1 - b; a++)
            if(a != window)
            {
                ...
                for(c = 0; c < vocab[last_word].character_size; c++)
                {
                    ...
                    //for(d = 0; d < layer1_size; d++) neu1[d] += synchar[d + char_id * layer1_size];
        ...
        if(cw)
        {
            ...
            for(d = 0; d < negative + 1; d++)
            {
                ...
                { // average context composition model
                    real f = 0, g = 0;
                    for(c = 0; c < layer1_size; c++)
                        f += (neu1[c] + neucomp[c]) * syn1neg[c + l2];
                    if(f > MAX_EXP)
                        g = (label - 1) * alpha;
                    else if(f < -MAX_EXP)
                        g = (label - 0) * alpha;
                    ...
                    for(c = 0; c < layer1_size; c++)
                        syn1neg[c + l2] += g * (neu1[c] + neucomp[c]);
                }
            }
            ...
            /*for(a = 0; a < char_list_cnt; a++)
            {
                ...
                for(c = 0; c < layer1_size; c++)
                {
                    if(average_sum == 1)
                        ...
                        synchar[c + char_id * layer1_size] += neucomp_grad[c];
                }
            }
            //再次更新字向量
            for(a = 0; a < char_list_cnt; a++)
            {
                ...
                {
                    if(average_sum == 1)
                        synchar[c + char_id * layer1_size] += neu1_grad[c] / (char_list_cnt + comp_list_cnt);
                    else
                        synchar[c + char_id * layer1_size] += neu1_grad[c];
                }
            }*/
            //更新部首向量
            for(a = 0; a < comp_list_cnt; a++)
            {
                ...
                {
                    if(average_sum == 1)
                        synchar[c + comp_id * layer1_size] += neucomp_grad[c] / (comp_list_cnt);
                    else
                        synchar[c + comp_id * layer1_size] += neucomp_grad[c];
                }
            }
        }
        sentence_position++;
